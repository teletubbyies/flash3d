...
CO3D_DATASET_ROOT = "../co3d_ansV1/" # Change this to where you saved preprocessed data
...
class CO3DDataset(SharedDataset):
    ...
    def load_example_id(self, example_id, intrin_path,
        ...
        #### new ###
        xyz_path = os.path.join(dir_path, "xyz_fg.npy")
        depth_path = os.path.join(dir_path, "depth_fg.npy")
        ##
        ...
        if example_id not in self.all_rgbs.keys():
            ...
            ### new
            xyzs = np.load(xyz_path)
            self.all_xyzs[example_id] = torch.from_numpy(xyzs)

            depths = np.load(depth_path)
            self.all_depths[example_id] = torch.from_numpy(depths)
            ...
            for cam_info in cam_infos:
                ...
                self.all_focals_pixels[example_id].append(torch.tensor([fov2focal(cam_info.FovX, self.cfg.data.training_resolution),
    ...
    def __getitem__(self, index):
        ...
        images_and_camera_poses = {
            ...
            "gt_xyzs": self.all_xyzs[example_id][frame_idxs].clone(),
